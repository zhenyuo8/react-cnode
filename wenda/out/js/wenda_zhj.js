	...
	function Link(){
		...
		$('.zhj_zan').on('click',function(){
			if(sign){
				...
				$(this).find(".zhj_zan_icon").css("background","url('http://172.16.0.135:8888/wenda/img/zan_c.png')")
				sign=false;
			}else{
				var num= parseInt($(this).find('i').text());
				num--;
				$(this).find('i').text(num);
				$(this).find(".zhj_zan_icon").css("background","url('http://172.16.0.135:8888/wenda/img/zan_kong.png')")
				sign=true;
			}
