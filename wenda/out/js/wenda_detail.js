...
window.onload = function(){
	...
	$(".answer_list").on("click","._huifu",function (event) {
		...
		if($(".answer_reply").css("display").indexOf("block")>-1){
			...
			$("._huifu img").attr("src","http://172.16.0.135:8888/wenda/img/arrow_down.png")
		}else{
			$(".answer_reply").css("display","block");
			$("._huifu img").attr("src","http://172.16.0.135:8888/wenda/img/arrow_up.png")
		}
	...
	$(".haveUse").click(
		function () {
			if($(this).find("img").attr("src").indexOf("zan_c")>-1){
				$(this).find("img").attr("src","http://172.16.0.135:8888/wenda/img/zan_kong.png")
				$(this).siblings("i").html(parseInt($(this).siblings("i").html())-1)
			}else{
				$(this).find("img").attr("src","http://172.16.0.135:8888/wenda/img/zan_c.png")
				$(this).siblings("i").html(parseInt($(this).siblings("i").html())+1)
			}
		}
	);
	...
	$(".follow").click(function () {
		if($(".follow").css("background").indexOf("red")>-1){
			$(".follow").css("backgroundImage","url(http://172.16.0.135:8888/wenda/img/xin.png)")
		}else{
			$(".follow").css("backgroundImage","url(http://172.16.0.135:8888/wenda/img/red_xin.png)")
		}
	})
}
function readFile(){
